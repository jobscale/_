...
(async () => {
  ...
  `;
    ...
    async checkEnemy(item) {
      const before = JSON.parse(await indexStore.getItem('player') || '{}');
      await indexStore.setItem('player', JSON.stringify({ ...before, ...item }));
    ...
    async onlineUsers(interval = 10) {
      ...
      const saveNames = await indexStore.getItem('names');
      if (saveNames !== names) {
        await indexStore.setItem('names', names);
      }
    ...
    async battleReport() {
      ...
      const known = await indexStore.getItem('report');
      if (known !== report) {
        await indexStore.setItem('report', report);
        const block = {
          type: 'section',
          fields: [
